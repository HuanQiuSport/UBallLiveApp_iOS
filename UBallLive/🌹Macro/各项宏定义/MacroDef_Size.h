...
#define ScreenWidth         MianScreen.width//获取屏幕宽高
#define ScreenHeight        MianScreen.height
#define MAINSCREEN_WIDTH    ScreenWidth
#define MAINSCREEN_HEIGHT   ScreenHeight

#define SCREEN_MAX_LENGTH   (MAX(ScreenWidth, ScreenHeight))
#define SCREEN_MIN_LENGTH   (MIN(ScreenWidth, ScreenHeight))

#define Screen_Width6 375.0
// 以iphone6 为比例
#define kUBLSize(value)     (1.0 * (value) * (MIN(ScreenWidth,ScreenHeight)) / Screen_Width6)
...
static inline CGFloat TabBarHeight(){
    //即时获取
    CGFloat tabBarHeight = UBLAppDelegate.sharedInstance.tabBarVC.tabBar.frame.size.height;
...
static inline CGFloat BottomSafeAreaHeight(){
    ...
    return TabBarHeight() - 49 - UBLAppDelegate.sharedInstance.tabBarVC.myTabBar.offsetHeight;
}
