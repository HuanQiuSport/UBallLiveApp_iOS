...
@interface STDPingServices ()
<
STSimplePingDelegate
>
{
    BOOL _hasStarted;
    BOOL _isTimeout;
...
+ (STDPingServices *)startPingAddress:(NSString *)address
                      callbackHandler:(void(^)(STDPingItem *item,
                                               NSArray *pingItems))handler {
...
- (void)cancel {
    [[self class] cancelPreviousPerformRequestsWithTarget:self
                                                 selector:@selector(_timeoutActionFired) object:nil];
    [self.simplePing stop];
    STDPingItem *pingItem = [[STDPingItem alloc] init];
...
- (void)st_simplePing:(STSimplePing *)pinger
  didStartWithAddress:(NSData *)address {
    ...
    [self performSelector:@selector(_timeoutActionFired)
               withObject:nil
               afterDelay:self.timeoutMilliseconds / 1000.0];
}
...
- (void)st_simplePing:(STSimplePing *)pinger
  didFailToSendPacket:(NSData *)packet
       sequenceNumber:(uint16_t)sequenceNumber
                error:(NSError *)error {
    [[self class] cancelPreviousPerformRequestsWithTarget:self
                                                 selector:@selector(_timeoutActionFired)
                                                   object:nil];
    _sequenceNumber = sequenceNumber;
    STDPingItem *pingItem = [[STDPingItem alloc] init];
    pingItem.ICMPSequence = _sequenceNumber;
