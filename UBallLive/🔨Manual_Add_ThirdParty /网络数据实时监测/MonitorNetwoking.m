...
@implementation MonitorNetwoking

static MonitorNetwoking *monitorNetwoking = nil;
+(MonitorNetwoking *)sharedInstance{
    @synchronized(self){
        if (!monitorNetwoking) {
            monitorNetwoking = MonitorNetwoking.new;
        }
...
-(SuspendLab *)rateLabel{
    if (!_rateLabel) {
        _rateLabel = SuspendLab.new;
    }return _rateLabel;
}
