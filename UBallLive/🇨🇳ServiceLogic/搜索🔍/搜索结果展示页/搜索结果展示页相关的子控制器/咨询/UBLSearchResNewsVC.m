//
//  UBLSearchResNewsVC.m
//  UBallLive
//
//  Created by Jobs on 2020/10/31.
//

#import "UBLSearchResNewsVC.h"

#import "UBLSearchBar.h"
#import "UBLSearchResView.h"

@interface UBLSearchResNewsVC ()
<
...
@implementation UBLSearchResNewsVC
...
- (nonnull __kindof UICollectionViewCell *)collectionView:(nonnull UICollectionView *)collectionView
    ...
    //    [cell.player.currentPlayerManager play];//建议不直接播，因为耗能
        ...
        UBLVideoCollectionViewCell_Style8 *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"UBLVideoCollectionViewCell_Style8" forIndexPath:indexPath];
        [cell richElementsInCellWithModel:self.dataMutArr[indexPath.row]];
        return cell;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    if (kind == UICollectionElementKindSectionHeader) {
        ...
        @weakify(self)
        [headerView actionBlockSearchCollectionReusableView:^(id data) {
            @strongify(self)
            NSLog(@"查看更多");
        }];
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        return [UBLVideoCollectionViewCell_Style1 cellSizeWithModel:nil];//CGSizeMake(170, 162);//4
    }else if (indexPath.section == 1){
        return [UBLVideoCollectionViewCell_Style8 cellSizeWithModel:nil];
    }else if (indexPath.section == 2){
        return [UBLVideoCollectionViewCell_Style5 cellSizeWithModel:nil];//CGSizeMake(MAINSCREEN_WIDTH, 96);//3
    }else if (indexPath.section == 3){
        if (indexPath.row == 0) {
            return [UBLVideoCollectionViewCell_Style9 cellSizeWithModel:nil];//CGSizeMake(MAINSCREEN_WIDTH - 2 * 11, 184);//3
        }else if (indexPath.row == 1){
            return [UBLVideoCollectionViewCell_Style7 cellSizeWithModel:nil];//CGSizeMake(MAINSCREEN_WIDTH - 2 * 11, 89);//3
        }else{
            return CGSizeZero;
        }
    }else{
...
-(NSMutableArray<NSString *> *)headerTitleStrMutArr{
    if (!_headerTitleStrMutArr) {
        ...
        [_headerTitleStrMutArr addObject:@"相关直播"];
        [_headerTitleStrMutArr addObject:@"相关赛程"];
        [_headerTitleStrMutArr addObject:@"相关用户"];
        [_headerTitleStrMutArr addObject:@"相关咨询"];
