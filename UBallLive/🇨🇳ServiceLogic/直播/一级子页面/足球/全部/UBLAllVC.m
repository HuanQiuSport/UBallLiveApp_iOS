...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
    ...
    if (kind == UICollectionElementKindSectionHeader) {
        UBLAllCollectionReusableView *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader
                                                                                      withReuseIdentifier:NSStringFromClass(UBLAllCollectionReusableView.class)
                                                                                             forIndexPath:indexPath];
        headerView.indexPath = indexPath;
        headerView.backgroundColor = self.bgCor;
        [headerView richElementsInCellWithModel:@{@"titleStr":self.headerTitleStrMutArr[0],
                                                  @"image":self.headerIMGMutArr[0]}];
        @weakify(self)
        [headerView actionBlockCollectionReusableView:^(UBLAllCollectionReusableView *data) {
            @strongify(self)
...
referenceSizeForHeaderInSection:(NSInteger)section {
    if (section == 0) {
        return  CGSizeZero;
    }return [UBLAllCollectionReusableView collectionReusableViewSizeWithModel:nil];
}
...
-(NSMutableArray<NSString *> *)headerTitleStrMutArr{
    if (!_headerTitleStrMutArr) {
        ...
        [_headerTitleStrMutArr addObject:@"赛程预告"];
